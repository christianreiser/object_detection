...
import heapq #changed to get second highest number
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for i in range(min(max_boxes_to_draw, boxes.shape[0])):               # for all boxes img
    if scores is None or scores[i] > min_score_thresh:                  # if score high 
        ...
        if not skip_labels:
          if not agnostic_mode:
            if classes[i] in category_index.keys():
              class_name = category_index[classes[i]]['name']           # class label
            else:
              class_name = 'N/A'
            display_str = str(class_name)
        if not skip_scores:
          if not display_str:
            display_str = '{}%'.format(int(100*scores[i]))              # score
          else:
    ...
    #print('chrei:2max:',heapq.nlargest(6, scores),'.  Label:',class_name) #chrei
